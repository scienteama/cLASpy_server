...
async def logout(res: Response, auth_service: Annotated[IAuthService,
                 Depends(lambda: ServiceProvider.get_auth_service())],):
...
async def check_session(request: Request, auth_service: Annotated[IAuthService, Depends(
        lambda: ServiceProvider.get_auth_service())],):
