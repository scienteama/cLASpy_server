...
def get_claspy_ml_core_version(claspyML_service: Annotated[IClaspyMLService, Depends(lambda: ServiceProvider.get_claspyml_service())]) -> str:
    version = claspyML_service.get_core_version()
    return version

@router.get("/algorithms/{name}/params", response_model=ApiResponse[Dict[str, AlgoParam]])
def get_algo_params(name: str, claspyML_service: Annotated[IClaspyMLService, Depends(lambda: ServiceProvider.get_claspyml_service())]):
...
def get_algorithms(claspyML_service: Annotated[IClaspyMLService, Depends(lambda: ServiceProvider.get_claspyml_service())]):
